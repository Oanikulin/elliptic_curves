...
BigInt my_pow(BigInt val, BigInt deg) {
    BigInt res(1);
    while (!deg.isZero()) {
        if ((deg % 2) == 1) {
            deg = deg - 1;
...
struct PointCurve {
    ...
    PointCurve(BigInt aa, BigInt bb, bool is = 0) {
        x = aa;
        y = bb;
        isINF = is;
    }

    PointCurve(BigInt aa) {
        x = aa;
        y = ((aa * aa * aa) % p + (aa * a) % p + b) % p;
        y = get_sqrt(y);
    }
};

PointCurve Doubling(PointCurve aa) {
    if ((aa.y).isZero() || aa.isINF)
        return PointCurve(aa.x, aa.y, 1);
    BigInt invv = (aa.y * 2) % p;
    invv = my_pow(invv, p - 2);
    BigInt k = (a + aa.x * aa.x * 3) % p;
    BigInt m = k * invv % p;
    BigInt x3 = ((m * m) % p - aa.x - aa.x + p + p) % p;
    BigInt y3 = ((p -  (m *(x3 - aa.x + p)) % p) - aa.y + p) % p;
    return PointCurve(x3, y3);
}

bool check(PointCurve aa) {
    if (aa.isINF)
        return 1;
    return ((aa.x * aa.x * aa.x + a * aa.x + b) % p == (aa.y * aa.y) % p);
}

PointCurve operator + (PointCurve aa, PointCurve bb) {
    assert(check(aa));
    assert(check(bb));
    if (aa.isINF)
        return bb;
    ...
    BigInt invv = (bb.x - aa.x + p) % p;
    invv = my_pow(invv, p - 2);
    BigInt k = (bb.y - aa.y + p) % p;
    BigInt m = (k * invv) % p;
    BigInt x3 = ((m * m) % p - aa.x - bb.x + p + p) % p;
    BigInt y3 = ((p -  (m *(x3 - aa.x + p)) % p) - aa.y + p) % p;
    return PointCurve(x3, y3);
}
...
PointCurve multiplier(PointCurve cur, BigInt deg) {
    if (deg == 0)
        return PointCurve(cur.x, cur.y, 1);
    if (deg == 1)
        return cur;
    if (deg % 2 == 1)
        return cur + multiplier(cur, deg - 1);
    return Doubling(multiplier(cur, deg / 2));
}
...
int main() {
    p = BigInt(string("115792089210356248762697446949407573530086143415290314195533631308867097853951"));
    a = p - 3;
    b = BigInt(string("41058363725152142129326129780047268409114441015993725554835256314039467401291"))% p;
    BigInt x = BigInt(string("48439561293906451759052585252797914202762949526041747995844080717082404635286")) % p;
    BigInt y = BigInt(string("36134250956749795798585127919587881956611106672985015071877198253568414405109")) % p;
    BigInt order = BigInt(string("115792089210356248762697446949407573529996955224135760342422259061068512044369")) % p;
    BigInt keyx, keyy;
    string s;
    cin >> s;
    keyx = BigInt(s);
    cin >> s;
    keyy = BigInt(s);
    PointCurve key(keyx, keyy);
    auto tmp = multiplier(key, 10);
    int n;
    cin >> n;
    for (int i = 0; i < n; ++i) {
        cin >> s;
