...
char number_to_char(intx::uint512 numm) {
    int num = static_cast<int>(numm);
    if (num < 10)
        return '0' + num;
    if (num < 36)
        return 'A' + num - 10;
...
intx::uint512 my_pow(intx::uint512 val, intx::uint512 deg) {
    intx::uint512 res(1);
    while (deg != 0) {
        if ((deg % 2) == 1) {
            deg = deg - 1;
...
struct PointCurveZ {
    ...
    PointCurveZ(intx::uint512 aa, intx::uint512 bb, intx::uint512 zz, bool is = 0) {
        x = aa;
        y = bb;
        z = zz;
        isINF = is;
...
PointCurveZ Doubling(PointCurveZ aa) {
    if (aa.y == 0 || aa.isINF)
        return PointCurveZ(aa.x, aa.y, aa.z, 1);
    intx::uint512 W = (aa.x + aa.z) % p * ((p + aa.x - aa.z) % p) % p * 3 % p;
    intx::uint512 S = aa.y * aa.z % p;
    intx::uint512 B = aa.y * aa.x % p * S % p;
    intx::uint512 H = (W * W % p + p - B * 8 % p) % p;
    intx::uint512 YY = (W * ((B * 4 + p - H) % p) % p - (aa.y * aa.y % p * S % p * S % p * 8 % p) + p) % p;
    return PointCurveZ(H * S % p * 2 % p, YY, S * S % p * S % p * 8 % p);
}

PointCurveZ operator + (PointCurveZ aa, PointCurveZ bb) {
    if (aa.isINF)
        return bb;
    if (bb.isINF)
        return aa;
    intx::uint512 U1 = (bb.y * aa.z) % p;
    intx::uint512 U2 = (aa.y * bb.z) % p;
    intx::uint512 V1 = (bb.x * aa.z) % p;
    intx::uint512 V2 = (aa.x * bb.z) % p;
    if (V1 == V2) {
        if (U1 == U2)
            return PointCurveZ(aa.x, aa.y, aa.z, 1);
        return Doubling(aa);
    }
    intx::uint512 U = (U1 - U2 + p) % p;
    intx::uint512 V = (V1 - V2 + p) % p;
    intx::uint512 W = (aa.z * bb.z) % p;
    intx::uint512 Vk = V * V % p * V % p;
    intx::uint512 A = (p + p + U * U % p * W % p - Vk - V * V % p * V2 % p * 2 % p) % p;
    intx::uint512 YY = (U * ((V * V % p * V2 % p - A + p) % p) % p - (Vk * U2) % p + p) % p;
    return PointCurveZ((V * A) % p, YY, (Vk * W) % p);
}
...
PointCurveZ Negate(PointCurveZ cur) {
    if (cur.isINF)
        return cur;
    if (cur.y == 0)
        return cur;
...
int main() {
    p = intx::from_string<intx::uint512>(std::string("115792089210356248762697446949407573530086143415290314195533631308867097853951"));
    a = p - 3;
    b = intx::from_string<intx::uint512>("41058363725152142129326129780047268409114441015993725554835256314039467401291");
    intx::uint512 x = intx::from_string<intx::uint512>("48439561293906451759052585252797914202762949526041747995844080717082404635286");
    intx::uint512 y = intx::from_string<intx::uint512>("36134250956749795798585127919587881956611106672985015071877198253568414405109");
    string s;
    cin >> s;
    intx::uint512 key = intx::from_string<intx::uint512>(s);
    int n;
    cin >> n;
    for (int i = 0; i < n; ++i) {
        cin >> s;
        PointCurveZ gen(0,0, 0), mes(0,0, 0);
        if (s == "Z") {
            gen = PointCurveZ(x, y, 1, 1);
        } else {
            string t;
            cin >> t;
            gen = PointCurveZ(intx::from_string<intx::uint512>(s), intx::from_string<intx::uint512>(t), 1);
        }
        cin >> s;
        if (s == "Z") {
            mes = PointCurveZ(x, y, 1, 1);
        } else {
            string t;
            cin >> t;
