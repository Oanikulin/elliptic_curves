...
struct Remainder{
    ...
    Remainder(unsigned long long a, unsigned int b) {
        val = a % b;
        p = b;
    }
    ...
    Remainder operator % (Remainder other) {
        return Remainder(((val - (((*this)/other).val * other.val) % p) + p) % p, p);
    }
...
vector<unsigned long long> uns_parse_string(string &s) {
...
int main() {
    ...
    getline(cin, s);
    getline(cin, s);
    vector<Remainder> fr = parse_string(s, p), sec;
    int mpw = fr.size();
    Polynomial<Remainder> f(fr);
    getline(cin, s);
    ...
    for (auto kk : digs) {
        ans = ans * (long long)p + (long long)kk;
        if (kk >= p)
            return 3;
    }
    while (!ans.isZero()) {
        s += number_to_char(ans % 64);
        ans /= 64;
    }
    cout << s;
}
