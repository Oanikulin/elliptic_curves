...
struct Remainder {
    ...
    Remainder(unsigned int a) {
        val = 1;
        p = a;
    }

    Remainder(unsigned int a, unsigned int b) {
        val = a;
        p = b;
    }
    ...
    Remainder operator * (unsigned int other) {
        return Remainder((val * 1ULL * other) % p, p);
    }
};

template<typename T>
T pow(T val, unsigned int deg) {
    T res(val.p);
    while (deg) {
        if (deg & 1) {
            --deg;
...
int main() {
    ...
    while (cin >> b >> a) {
        digs.push_back(decrypt(Remainder(a, p), Remainder(b, p), p, k).val);
    }
    reverse(digs.begin(), digs.end());
    for (auto k : digs)
        ans = ans * (long long)p + (long long)k;
    while (!ans.isZero()) {
